  ...
  return (
    // ye simple hai, component ke andar loop ok,,, but loop ke andar component ko render karane me roblem aati hai
     <div key={v4()}className="card">  
          <h3 style={ {textalign:'center' } }>{ props?.files[0].subjectName}</h3>
          {
              props?.files.map( (file) => (        
                    <a style={ {textDecoration: 'none', color:'black', fontSize:'18px' , padding:'20px'} } target='_blank' href={file.url} >{ file.fileName.split(".pdf")[0] + ".pdf" } <br></br></a>
              ))
          }
     </div>
   /* ----------------------------------------- */
